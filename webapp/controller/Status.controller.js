sap.ui.define([
  ...
  return Controller.extend("com.expensemanagement.expensemanagement.controller.Status", {
    formatter: formatter,
    onInit() {
      this._aFilters = [];
      this._sSearch = "";
...
openDisplayDialog(sTitle, sMessage, bShowActions = false) {
    // Create model
    const oDialogModel = new sap.ui.model.json.JSONModel({
        title: sTitle,
        message: sMessage,
        showActions: bShowActions
    });

    this.getView().setModel(oDialogModel, "dialogModel");

    // Lazy load fragment
    if (!this._oDisplayDialog) {
        this._oDisplayDialog = sap.ui.xmlfragment(
            "com.expensemanagement.expensemanagement.fragments.DisplayDialog",
            this
        );
        this.getView().addDependent(this._oDisplayDialog);
    }

    this._oDisplayDialog.open();
},



onDialogCancel() {
    this._oDisplayDialog.close();
...
onShowReason(oEvent) {
    ...
    const sReason = oData.reason || "No reason provided";

    this.openDisplayDialog("Request Reason", sReason, false);
}
